...
class Form:
    ...
    async def upload_truck_data(self,plate, technician, order, cleaning, water, spare_tire, oil, jack, cross_wrench, fire_extinguisher, padlock, comment,):
        db = Database(host="localhost", user="root", password="h1h2h39zho", database="camionetas")
        query = f'''INSERT INTO informacion_camionetas (
                Patente,
                Tecnico,
                Orden,
                Limpieza,
        ...
        try:
            # Ejecutar la consulta SQL utilizando el método execute de la clase Database
            await db.execute(query)
            print("Datos subidos exitosamente a la base de datos.")

            con="SELECT * FROM informacion_camionetas"
            resultados=await db.execute(con)
    ...
    async def send_form(self,e):
        
        await self.upload_truck_data(
    ...
    def ui_send_button(self):
        if int(self.txt_number.value)>=3:
            ...
            self.send_button.on_click=lambda e:asyncio.run(self.send_form(e))
